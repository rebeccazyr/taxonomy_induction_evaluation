#!/usr/bin/env python3
"""
Complete EMNLP 2024 PDF download script
...
# Configuration
BASE_URL = "https://aclanthology.org/2024.emnlp-main."
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
SAVE_DIR = os.path.join(BASE_DIR, "data", "emnlp2024", "pdf")
...
def scan_existing_files():
    ...
    if os.path.exists(SAVE_DIR):
        for filename in os.listdir(SAVE_DIR):
            if filename.endswith('.pdf') and filename.startswith('2024.emnlp-main.'):
                # Extract paper number from filename
                match = re.search(r'2024\.emnlp-main\.(\d+)\.pdf', filename)
                if match:
...
def download_pdf(paper_num: int, retries: int = 3) -> bool:
    ...
    filename = f"2024.emnlp-main.{paper_num}.pdf"
...
def main():
    ...
    print("Starting download of EMNLP 2024 main conference papers...")
